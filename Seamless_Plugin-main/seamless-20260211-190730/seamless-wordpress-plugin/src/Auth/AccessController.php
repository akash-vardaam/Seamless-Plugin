...
class AccessController
{
    ...
    {
        // Never restrict access during Elementor editor/preview sessions.
        // Elementor embeds the page in an iframe with ?elementor-preview=<id>
        // Calling status_header(403) here would corrupt the nonce context for
        // all Elementor REST API calls, causing rest_cookie_invalid_nonce errors.
        if (isset($_GET['elementor-preview']) || (isset($_GET['action']) && $_GET['action'] === 'elementor')) {
            return;
        }

        // Never restrict during REST API requests — REST has its own auth.
        if (defined('REST_REQUEST') && REST_REQUEST) {
            return;
        }

        // Never restrict during admin-ajax requests.
        if (defined('DOING_AJAX') && DOING_AJAX) {
            return;
        }

        // Never restrict in the wp-admin Elementor editor page.
