...
class UserProfile
{
    ...
    public function get_user_orders(string $email)
    {
        ...
        $result = $this->fetch_data('dashboard/orders', ['email' => $email]);
        ...
        if ($result['success']) {
            ...
            // Handle paginated response - extract orders from data.data
            if (isset($ordersData['data']) && is_array($ordersData['data'])) {
                $ordersData = $ordersData['data'];
                // error_log('Seamless Order History - Extracted Orders: ' . count($ordersData) . ' orders found');
            }
     ...
     * Fetches fresh data from API on every request
     *
    ...
    private function find_user_membership(string $membership_id, string $email): ?array
    {
        if (empty($membership_id) || empty($email)) {
            return null;
        }

        $memberships_result = $this->get_user_memberships($email);
        if (empty($memberships_result['success'])) {
            return null;
        }

        foreach (['current', 'history'] as $membership_group) {
            foreach ((array)($memberships_result['data'][$membership_group] ?? []) as $membership) {
    ...
    private function is_group_membership(array $membership): bool
    {
        return !empty($membership['plan']['is_group_membership'])
            || ($membership['membership_type'] ?? '') === 'group_member'
            || !empty($membership['group_role']);
    }

    private function is_group_membership_owner(array $membership): bool
    {
        return !empty($membership['is_owner'])
            || strtolower((string)($membership['group_role'] ?? ($membership['role'] ?? ''))) === 'owner';
    }

    private function validate_group_owner_action(string $membership_id, string $email, string $action_label): array
    {
        $membership = $this->find_user_membership($membership_id, $email);
        if (!$membership) {
            return ['success' => false, 'message' => 'Membership not found or access denied'];
        }

        if ($this->is_group_membership($membership) && !$this->is_group_membership_owner($membership)) {
            return ['success' => false, 'message' => sprintf('Only group owners can %s this membership', $action_label)];
        }
    ...
    public function upgrade_membership(string $new_plan_id, string $membership_id, string $email)
    {
        ...
        if (!empty($membership_id)) {
            $owner_validation = $this->validate_group_owner_action($membership_id, $email, 'upgrade');
            if (empty($owner_validation['success'])) {
    ...
    public function downgrade_membership(string $new_plan_id, string $membership_id, string $email)
    {
        ...
        if (!empty($membership_id)) {
            $owner_validation = $this->validate_group_owner_action($membership_id, $email, 'downgrade');
            if (empty($owner_validation['success'])) {
                return $owner_validation;
            }
        }
    ...
    public function cancel_membership(string $membership_id, string $email)
    {
        ...
        $owner_validation = $this->validate_group_owner_action($membership_id, $email, 'cancel');
        if (empty($owner_validation['success'])) {
            return $owner_validation;
        }
     ...
     * 
     * @param string $plan_id Plan ID to renew
     * @param string $email User email
     * @return array Renew result with Stripe checkout URL
     */
    public function renew_membership(string $plan_id, string $email)
    {
        if (empty($plan_id) || empty($email)) {
            return ['success' => false, 'message' => 'Plan ID and email are required'];
        }

        // Validate current user can renew
        $current_user = wp_get_current_user();
        if ($current_user->user_email !== $email && !current_user_can('administrator')) {
            return ['success' => false, 'message' => 'Unauthorized to renew this membership'];
        }
        ...
        try {
            ...
            $url = $this->domain . '/api/dashboard/memberships/renew/' . $plan_id;
            ...
            // Log the full response for debugging
            error_log('Seamless Renew Membership Response - HTTP Code: ' . $http_code);
            error_log('Seamless Renew Membership Response - Body: ' . print_r($body, true));

            if (!in_array($http_code, [200, 201]) || !is_array($body)) {
                $message = $body['message'] ?? 'API error occurred. Code: ' . $http_code;
                error_log('Seamless Renew Membership Error - ' . $message);
                throw new \Exception($message);
            }
            ...
            return [
                'success' => true,
                'data' => $body['data'] ?? $body,
                'message' => $body['message'] ?? 'Renewal initiated successfully'
            ];
        } catch (\Exception $e) {
            error_log('Seamless UserProfile API Error (renew membership): ' . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage(), 'data' => []];
        }
    }
     ...
     */
    public function get_user_organization(string $email)
    {
        if (empty($email)) {
            return ['success' => false, 'message' => 'Email is required', 'data' => []];
        }

        $result = $this->fetch_data('dashboard/organization', ['email' => $email]);

        // error_log('Seamless UserProfile API Response (dashboard/organization): ' . print_r($result, true));
    ...
    /**
     * Request removal from a group membership.
     *
     * @param string $membership_id Membership ID
     * @param string $email User email
     * @return array Response
     */
    public function request_group_removal(string $membership_id, string $email)
    {
        if (empty($membership_id) || empty($email)) {
            return ['success' => false, 'message' => 'Membership ID and email are required'];
        }
        ...
        $current_user = wp_get_current_user();
        if ($current_user->user_email !== $email && !current_user_can('administrator')) {
            return ['success' => false, 'message' => 'Unauthorized to request removal from this membership'];
        }
        ...
        if (!$this->is_group_membership($membership)) {
            return ['success' => false, 'message' => 'Removal requests are only available for group memberships'];
        }

        if ($this->is_group_membership_owner($membership)) {
            return ['success' => false, 'message' => 'Group owners cannot request removal from this membership'];
        }

        $group_role = strtolower((string)($membership['group_role'] ?? ($membership['role'] ?? '')));
        if (!in_array($group_role, ['admin', 'member'], true)) {
            return ['success' => false, 'message' => 'Only group admins or members can request removal'];
        }
        ...
        try {
            ...
            $url = $this->domain . '/api/dashboard/memberships/' . $membership_id . '/request-removal';
            $data = [
                'email' => $email,
            ];
            ...
            if (!in_array($http_code, [200, 201]) || !is_array($body)) {
                $message = is_array($body) ? ($body['message'] ?? 'Failed to request removal') : 'Failed to request removal. Code: ' . $http_code;
                throw new \Exception($message);
            }
            ...
            return [
                'success' => true,
                'data' => $body['data'] ?? $body,
                'message' => $body['message'] ?? 'Removal request sent successfully',
            ];
        } catch (\Exception $e) {
            error_log('Seamless UserProfile API Error (request group removal): ' . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage(), 'data' => []];
        }
    }

    /**
     * Resend group membership invite
     *
     * @param string $membership_id Group membership ID
     * @param string $member_id Member ID to resend invite to
     * @return array Response
     */
    public function resend_group_invite(string $membership_id, string $member_id)
    {
        try {
            $access_token = $this->get_access_token();
            if (!$access_token) {
            ...
            $url = $this->domain . '/api/dashboard/group/' . $membership_id . '/members/' . $member_id . '/resend-invite';

            error_log('Seamless Resend Invite Request - URL: ' . $url);
            ...
            error_log('Seamless Resend Invite Response - HTTP Code: ' . $http_code);
            error_log('Seamless Resend Invite Response - Body: ' . print_r($body, true));

            if (!in_array($http_code, [200, 201]) || !is_array($body)) {
                $message = $body['message'] ?? 'Failed to resend invite. Code: ' . $http_code;
                throw new \Exception($message);
            }

            return ['success' => true, 'data' => $body['data'] ?? $body, 'message' => $body['message'] ?? 'Invite resent successfully'];
        } catch (\Exception $e) {
            error_log('Seamless UserProfile API Error (resend invite): ' . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage(), 'data' => []];
        }
    }
     ...
     * @return array Response
     */
    public function add_group_members(string $membership_id, array $members)
    {
        try {
            $access_token = $this->get_access_token();
            if (!$access_token) {
            ...
            $url = $this->domain . '/api/dashboard/group/' . $membership_id . '/members';

            error_log('Seamless Add Group Members Request - URL: ' . $url);
            error_log('Seamless Add Group Members Request - Data: ' . print_r($members, true));
            ...
            $response = wp_remote_post($url, [
                ...
                'body' => json_encode(['members' => $members]),
            ]);
            ...
            error_log('Seamless Add Group Members Response - HTTP Code: ' . $http_code);
            error_log('Seamless Add Group Members Response - Body: ' . print_r($body, true));

            if (!in_array($http_code, [200, 201]) || !is_array($body)) {
                $message = $body['message'] ?? 'Failed to add members. Code: ' . $http_code;
                throw new \Exception($message);
            }

            return ['success' => true, 'data' => $body['data'] ?? $body, 'message' => $body['message'] ?? 'Members added successfully'];
        } catch (\Exception $e) {
            error_log('Seamless UserProfile API Error (add group members): ' . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage(), 'data' => []];
        }
    }
     ...
     *
     * @param string $membership_id Group membership ID
     * @param string $member_id Member ID to remove
     * @return array Response
     */
    public function remove_group_member(string $membership_id, string $member_id)
    {
        try {
            $access_token = $this->get_access_token();
            if (!$access_token) {
            ...
            $url = $this->domain . '/api/dashboard/group/' . $membership_id . '/members/' . $member_id;

            error_log('Seamless Remove Group Member Request - URL: ' . $url);
            ...
            error_log('Seamless Remove Group Member Response - HTTP Code: ' . $http_code);

            if (!in_array($http_code, [200, 201, 204]) || ($body !== null && !is_array($body))) {
                $message = is_array($body) ? ($body['message'] ?? 'Failed to remove member') : 'Failed to remove member. Code: ' . $http_code;
                throw new \Exception($message);
            }

            return ['success' => true, 'data' => $body['data'] ?? [], 'message' => $body['message'] ?? 'Member removed successfully'];
        } catch (\Exception $e) {
            error_log('Seamless UserProfile API Error (remove group member): ' . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage(), 'data' => []];
        }
    }
     ...
     * @return array Response
     */
    public function change_member_role(string $membership_id, string $member_id, string $role)
    {
        try {
            $access_token = $this->get_access_token();
            if (!$access_token) {
            ...
            $url = $this->domain . '/api/dashboard/group/' . $membership_id . '/members/' . $member_id . '/role';
            ...
            if (!in_array($http_code, [200, 201]) || !is_array($body)) {
                $message = is_array($body) ? ($body['message'] ?? 'Failed to change role') : 'Failed to change role. Code: ' . $http_code;
                throw new \Exception($message);
            }

            return ['success' => true, 'message' => $body['message'] ?? 'Role updated successfully'];
        } catch (\Exception $e) {
            error_log('Seamless UserProfile API Error (change member role): ' . $e->getMessage());
