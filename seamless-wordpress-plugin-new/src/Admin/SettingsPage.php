...
class SettingsPage
{
	...
	public function __construct()
	{
		...
		add_action('update_option_seamless_product_list_endpoint', [$this, 'schedule_rewrite_flush']);
...
?>
	...
	public function register_settings(): void
	{
		...
		register_setting('seamless_endpoints_group', 'seamless_product_list_endpoint', [
			'sanitize_callback' => 'sanitize_title_with_dashes'
		]);
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<code><?php echo esc_url(home_url('/')); ?></code>
						<input type="text" name="seamless_product_list_endpoint"
							value="<?php echo esc_attr(get_option('seamless_product_list_endpoint', 'shop')); ?>"
							class="regular-text" />
					</td>
				</tr>
				<tr>
	...
	?>
		<div class="seamless-section-container">
			<ul class="seamless-shortcodes-list">
				<li>
					...
					<span class="shortcode-container" style="display: inline-flex; vertical-align: middle;">
						<code class="seamless-code-block">[seamless_product_list]</code>
						<button type="button" class="copy-shortcode-btn" title="Copy shortcode" data-shortcode="[seamless_product_list]">
							<span class="dashicons dashicons-admin-page"></span>
						</button>
					</span>
				</li>
				<li>
					...
					<span class="shortcode-container" style="display: inline-flex; vertical-align: middle;">
						<code class="seamless-code-block">[seamless_single_product slug="sample-product"]</code>
						<button type="button" class="copy-shortcode-btn" title="Copy shortcode" data-shortcode='[seamless_single_product slug="sample-product"]'>
							<span class="dashicons dashicons-admin-page"></span>
						</button>
					</span>
				</li>
		...
		<?php if (!$this->auth->is_authenticated()): ?>
			<div class="seamless-api-notice" style="padding: 20px; background: #fff; border: 1px solid #ccd0d4; margin-top: 10px;border-radius: 12px;">
				<p>Please authenticate to view shop products.</p>
			</div>
		<?php else: ?>
			<div class="seamless-search-bar">
				<div class="seamless-search-input">
				...
				<button type="button" class="button seamless-search-reset" id="seamless-shop-reset" style="display: none;">
					<span class="dashicons dashicons-dismiss"></span> Clear
				</button>
			</div>
			<div class="seamless-table-area">
	...
	?>
		<script>
			jQuery(document).ready(function($) {
				...
				// Check for URL-based flash messages unless the outer Seamless shell already handled them
				var urlParams = new URLSearchParams(window.location.search);
				if (!window.__seamlessAdminFlashHandled) {
					var handledFlashMessage = false;
					...
					if (handledFlashMessage) {
						window.__seamlessAdminFlashHandled = true;
					}
				}
				...
				$('form[action="options.php"]').each(function() {
					...
					// Add hidden inputs for the current location state
...
<?php
	...
	public function maybe_flush_permalinks($option, $old_value, $value): void
	{
		$target_options = [
			...
			'seamless_product_list_endpoint',
