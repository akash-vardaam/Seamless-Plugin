jQuery(document).ready(function ($) {
  ...
  const isShopTab =
    $('.nav-tab-active[data-tab="shop"]').length > 0 ||
  ...
  function getProductCategoriesLabel(product) {
    if (!product.categories || !Array.isArray(product.categories)) {
      return "-";
    }

    const labels = product.categories
      .map((category) => category?.name || category?.label || category?.slug || "")
      .filter(Boolean);
  ...
  function renderShopTable() {
    ...
    pageItems.forEach((product, index) => {
      const shortcode = `[seamless_single_product slug="${product.slug}"]`;
      const row = `
        <tr>
          <td>${start + index + 1}</td>
          <td><strong>${escapeHtml(product.title || "Untitled Product")}</strong></td>
          <td>${escapeHtml(getProductCategoriesLabel(product))}</td>
          <td>${escapeHtml(product.priceLabel || "-")}</td>
          <td><span class="seamless-status-badge rfc-compliant">Physical</span></td>
          <td><span class="seamless-status-badge enabled">Available</span></td>
          <td>
            <div class="shortcode-container">
              <code class="seamless-code-block">${escapeHtml(shortcode)}</code>
              <button type="button" class="copy-shortcode-btn" data-shortcode='${escapeHtml(shortcode)}' title="Copy">
                <span class="dashicons dashicons-admin-page"></span>
              </button>
            </div>
          </td>
  ...
  async function loadProducts(force = false) {
    ...
    const loader = $(
      '<div class="seamless-admin-loader"><div class="seamless-admin-spinner"></div><div class="seamless-admin-loading-text">Loading Products...</div></div>'
    );
    $(".seamless-table-area").has(shopTableBody).append(loader);
    ...
    try {
      if (!window.SeamlessAPI) {
        shopTableBody.html(
          '<tr><td colspan="7" style="text-align:center; color:red;">API Client not loaded. Refresh page.</td></tr>'
        );
        loader.remove();
        return;
      }

      allProducts = await window.SeamlessAPI.getAllShopProducts();
      filteredProducts = allProducts;
      shopPage = 1;
      shopTableBody.data("loaded", "true");
      renderShopTable();
    } catch (error) {
      console.error("Seamless Admin: Error loading products", error);
      shopTableBody.html(
        `<tr><td colspan="7" style="text-align:center; color:red;">Error loading products: ${
          error.message || "Unknown error"
        }</td></tr>`
      );
    } finally {
  ...
  $("#seamless-shop-search").on("input", function () {
    ...
    if (term.length > 0) {
      $("#seamless-shop-reset").show();
    } else {
      $("#seamless-shop-reset").hide();
    }
    ...
    filteredProducts = allProducts.filter((product) => {
      const haystack = [
        ...
        product.slug,
        product.sku,
        getProductCategoriesLabel(product),
      ]
        .join(" ")
        .toLowerCase();
  ...
  $("#seamless-shop-reset").on("click", function (event) {
    event.preventDefault();
