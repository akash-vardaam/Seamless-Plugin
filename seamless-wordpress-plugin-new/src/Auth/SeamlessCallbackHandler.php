...
class SeamlessCallbackHandler
{
    ...
    public function handle(WP_REST_Request $req): WP_Error|WP_User
    {
        ...
        $close_window = !empty($_SESSION[self::SSO_PREFIX]['pkce'][$nonce]['close_window']);
        ...
        try {
            ...
            if ($close_window) {
                $this->render_close_window_response();
            }
    ...
    private function render_close_window_response(): void
    {
        if (!headers_sent()) {
            status_header(200);
        ...
        echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Login successful</title></head><body><script>window.close();</script><p>Login successful. You can close this window.</p></body></html>';
        exit;
    }
    ...
    private function insert_update_wp_user(array $user_data, array $tokens): WP_User|WP_Error
    {
        ...
        // Persist AMS user id for cart identity flows that use user_id instead of bearer token.
        if (!empty($user['id'])) {
            update_user_meta($wp_user->ID, 'seamless_ams_user_id', sanitize_text_field((string) $user['id']));
        }
