...
if ($startDate) {
  try {
    // Both event types now use formatted_start_date which is in local time
  ...
  if ($endDate) {
    try {
      // Both event types now use formatted_end_date which is in local time
...
// Collect all tickets from associated events for group events
$all_tickets = [];
if ($event_type === 'group_event' && !empty($event['associated_events'])) {
  foreach ($event['associated_events'] as $associated_event) {
    if (!empty($associated_event['tickets'])) {
...
$calendar_description = !empty($excerpt)
  ...
  <div class="event-main-content">
    <div class="event-header">
      ...
      <div class="event-header-text">
        ...
        if (!empty($excerpt)) :
        ?>
          <div class="event-excerpt-description">
      ...
      ?>
        <div class="accordion-item">
          <button class="accordion-header">
            ...
            <?= count($organizers) === 1 ? 'Organizer Details' : 'Organizer Details'; ?>
          </button>
          <div class="accordion-body">
            <div class="event-organizers-list">
              <?php foreach ($organizers as $organizer) :
                $org_name  = trim($organizer['full_name'] ?? (($organizer['first_name'] ?? '') . ' ' . ($organizer['last_name'] ?? '')));
                $org_email = $organizer['email'] ?? '';
                $org_phone = $organizer['phone_number'] ?? '';
                $org_phone_type = ucfirst($organizer['phone_type'] ?? '');
              ?>
                <div class="event-organizer-card">
                  ...
                  <?php if ($org_name) : ?>
                    <div class="event-organizer-row">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                        ...
                        stroke-linecap="round" stroke-linejoin="round" class="organizer-icon">
                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                        <circle cx="12" cy="7" r="4" />
                      </svg>
                      <span class="organizer-name"><?= esc_html($org_name); ?></span>
                    </div>
                  <?php endif; ?>
                  ...
                  <?php if ($org_email) : ?>
                    <div class="event-organizer-row">
                      ...
                      <a href="mailto:<?= esc_attr($org_email); ?>" class="organizer-email">
                        <?= esc_html($org_email); ?>
                      </a>
                    </div>
                  <?php endif; ?>
                  ...
                  <?php if ($org_phone) : ?>
                    <div class="event-organizer-row">
                      <?php if ($org_phone_type === 'Mobile' || $org_phone_type === 'Cell') : ?>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                          fill="none" stroke="currentColor" stroke-width="2"
                          stroke-linecap="round" stroke-linejoin="round" class="organizer-icon">
                      ...
                      <?php else : ?>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                          fill="none" stroke="currentColor" stroke-width="2"
                          stroke-linecap="round" stroke-linejoin="round" class="organizer-icon">
                          <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.18 2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" />
                        </svg>
                      <?php endif; ?>
                      <a href="tel:<?= esc_attr($org_phone); ?>" class="organizer-phone">
                        <?= esc_html($org_phone); ?>
                      </a>
                      <?php if ($org_phone_type) : ?>
                        <span class="organizer-phone-type">(<?= esc_html($org_phone_type); ?>)</span>
                      <?php endif; ?>
                    </div>
                  <?php endif; ?>
    ...
    <div class="event-info-card">
      ...
      if (!empty($venue_name)) {
        if (!empty($google_map_url)) {
          $location_html .= '<a href="' . esc_url($google_map_url) . '" target="_blank" rel="noopener noreferrer" class="venue-link" style="text-decoration: none; color: inherit;">' . esc_html($venue_name) . '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-left: 4px; vertical-align: middle;"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></a>';
        } else {
